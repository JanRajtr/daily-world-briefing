...
A fully autonomous, no-OpenAI-API daily briefing. GitHub Actions downloads public observations, computes a transparent weighted risk score, creates a Kobo/Instapaper-friendly static HTML article, preserves a dated archive, deploys it to GitHub Pages, and optionally submits the unique URL to Instapaper.

## What it produces

- `https://YOUR-USER.github.io/YOUR-REPO/reports/YYYY-MM-DD.html` — one immutable URL per report date
- `.../index.html` — newest-first archive
- `.../latest.html` — redirect to the report generated by the latest run
...
6. When the run is green, open the deployment URL shown in the `deploy` job and verify the dated article and archive.
...
The scheduled run is Monday–Friday at **06:15 UTC** (07:15 in Prague winter time, 08:15 in summer time). Edit the cron line in `.github/workflows/daily-briefing.yml` if you prefer another time. GitHub schedules are UTC and may start a little late during periods of high load.
...
4. Generates the dated HTML, tiny JSON metadata record, newest-first archive and `latest.html`. An existing dated report is preserved rather than recalculated with revised data.
5. Commits the generated `site/` files back to the repository using GitHub's built-in token, preserving history.
...
Scheduled runs always notify Instapaper. Manual runs notify only when the workflow input is enabled. Re-running the same date preserves the existing report and URL; Instapaper marks an existing item unread rather than creating a duplicate.
...
Then open `http://localhost:8000/reports/2025-02-03.html`.
