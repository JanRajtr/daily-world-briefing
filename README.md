...
Rešerše citátu a buddhistického učení běží odděleně od ostatního denního obsahu na `groq/compound-mini`, který provede nejvýše jeden webový nástrojový krok; rozsáhlejší doplňky zůstávají na `groq/compound`. Oba požadavky používají jen `web_search`, aby načítání celých webů nepřeplnilo interní kontext. Krátká reflexe má limit 1 000 výstupních tokenů a obsáhlejší doplňky 3 200, takže požadavky zbytečně nerezervují výchozí maximum modelu. Po úspěšné reflexi klient před doplňky čeká nejméně 65 sekund, nebo déle podle serverové hlavičky `x-ratelimit-reset-tokens`; skutečný limit, zbývající tokeny a reset ukládá do metadat. Přechodné minutové chyby se až dvakrát zopakují. U denního limitu TPD se provede jediný odložený pokus po pěti minutách pouze tehdy, když server odhaduje dostupnou kapacitu do pěti minut; delší TPD reset ani druhé selhání se neopakuje, protože i neúspěšná Compound orchestrace může spotřebu dále zvýšit. Typ limitu a serverem uvedený reset se uloží do metadat a chybějící obsah se nevymýšlí. Chyby požadavku jako HTTP 413 se také neopakují. Po minutovém HTTP 429 klient vyčká nejméně 65 sekund jako konzervativní minimum; delší serverový `Retry-After`, text chyby nebo hlavička `x-ratelimit-reset-tokens` má přednost. Ani tato prodleva nemůže garantovat volnou kapacitu, pokud stejný organizační limit mezitím spotřebovává jiný proces. U ostatních přechodných chyb klient použije exponenciální prodlevu. Stav, model, počet pokusů, tokenový limit a bezpečný důvod případného vynechání se ukládají do `site/report.json`; chyby Groq jsou současně viditelné v logu GitHub Actions.
